...
const stock_daily_entity_1 = require("../entity/stock-daily.entity");
...
async function run() {
    ...
    async function updateStockSummary() {
        ...
        for (let i = 0; i < all.length; i++) {
            const array = all[i].filter((it) => !(it.stock_name || '').includes('退市'));
    ...
    const delRes = await db_1.dataSource.getRepository(stock_daily_entity_1.StockDailyTab).delete({
        date
    });
    console.log(`删除旧数据`, delRes.affected);
    for (let i = 0; i < data.length; i++) {
        const array = data[i].map((it) => {
            return {
                stock_name: it['f14'],
                stock_code: it['f12'],
                price: Number.isNaN(Number(it['f2'])) ? 0 : Number(it['f2']),
        ...
        }).filter((it) => !(it.stock_name || '').includes('退市'));
        await db_1.dataSource.getRepository(stock_daily_entity_1.StockDailyTab).save(array);
    }
    console.log('开始更新汇总表');

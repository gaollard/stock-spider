...
(async function startRun() {
    console_1.default.log(222);
    await (0, db_1.initDb)();
    console_1.default.log(333);
    // 查询所有基金
    let list = await db_1.dataSource.getRepository(funder_entity_1.Funder).find({});
    for (let i = 0; i < list.length; i++) {
        const record = list[i];
        const funder_code = record.code;
        const funder_title = record.title;
        console_1.default.log(i, funder_code, funder_title);
        if (fs_1.default.existsSync(`resource/2024-03-31/funder_stock/${funder_code}.json`)) {
            continue;
        }
        const data = await query_by_ccmx_api(funder_code, funder_title);
        fs_1.default.writeFileSync(`resource/2024-03-31/funder_stock/${funder_code}.json`, JSON.stringify(data));
    }
})();
/**
 * 查询基金的持仓明细
 */
async function query_by_ccmx_api(fund_code, fund_name) {
    ...
    const list = [];
    $('.box table tbody').each((i, el) => {
        if (i === 0) {
            $(el).find("tr").each((j, el) => {
                if (j < 30) {
                    const item = {
